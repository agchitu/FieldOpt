...
po::variables_map createVariablesMap(int argc, const char **argv) {
  ...
  desc.add_options()
      ...
      ("heel_md,hm", po::value<double>(), "heel measured depth")

      ("toe_md,tm", po::value<double>(), "toe measured depth")
  ...
  { // Print help if --help present or input file/output dir not present
    cout << "Usage: ./wicalc_ri --grid gridpath --heel x1 y1 z1 "
        "--toe x2 y2 z2 --heel_md d --toe_md d --radius r --skin-factor s [options]" << endl;
  ...
  if (!vm.count("well-filedef") && (!vm.count("heel_md") || !vm.count("toe_md"))) {
    cerr << "Error: You must provide heel and toe measured depth." << endl;
    exit(EXIT_FAILURE);
  }
