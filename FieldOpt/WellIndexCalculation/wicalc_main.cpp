...
int main(int argc, const char *argv[])
{
    ...
    map<string, vector<IntersectedCell>> all_wells_indices;
    for (int iWell = 0; iWell < wells.size(); ++iWell)
   {
        vector<IntersectedCell> well_indices;  
        wic.ComputeWellBlocks(well_indices, wells.at(iWell));
        all_wells_indices[wells[iWell].wellname] = well_indices;	 
    }
    
    // Print as a COMPDAT table if the --compdat/-c flag was given
    if (vm.count("compdat")) 
    {
        printCompdat(all_wells_indices);
    }
    // Otherwise, print as a CSV table
    else 
    {
        printCsv(all_wells_indices);
    }

    if (vm.count("debug") > 0) {
        printDebug(all_wells_indices);
    }
